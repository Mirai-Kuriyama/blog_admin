  ...
  <div class="article-add">
    <Form :model="formTop" label-position="top" ref="formValidate" :rules="ruleValidate">
      ...
      <FormItem prop="content">
        <vue-xeditor :id="editorId" v-model="formTop.content"></vue-xeditor>
      </FormItem>
      <FormItem label="过审后自动发布" prop="auto_pub">
        <i-switch v-model="formTop.auto_pub" size="large">
          <span slot="open">自动</span>
          <span slot="close">手动</span>
        </i-switch>
      </FormItem>
      <FormItem>
        <Button type="primary" @click="handleSubmit(1)">提交审核</Button>
...
export default {
  ...
  methods: {
    ...
    getCat() {
      this.$http.get("/api/category/get").then(res => {
        res = res.data;
        if (res.msg == "ok") {
